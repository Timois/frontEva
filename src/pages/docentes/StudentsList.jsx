...
export const StudentsList = () => {
  ...
  // 🔎 Filtrar estudiantes antes de paginarlos
  const filteredStudents = students.filter((s) => {
    const term = searchTerm.toLowerCase();

    const ci = s.ci?.toString().toLowerCase() || "";
    const name = s.name?.toLowerCase() || "";
    const paternal = s.paternal_surname?.toLowerCase() || "";
    const maternal = s.maternal_surname?.toLowerCase() || "";

    return (
      ci.includes(term) ||
      name.includes(term) ||
      paternal.includes(term) ||
      maternal.includes(term)
    );
  });

  // Actualizar paginación según la búsqueda
